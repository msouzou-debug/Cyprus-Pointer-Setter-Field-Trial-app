...
Replace `index.html` at the host, then **bump `CACHE` in `sw.js`** — it is at `"koad-v2"`, so the next release is `"koad-v3"` — and upload that too. Without the version bump the installed phones keep serving the old copy from cache.
