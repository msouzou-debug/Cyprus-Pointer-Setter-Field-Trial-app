...
## Έξι σημεία που έμειναν ελληνικά, και ένα σήμα που λείπει

Ο κριτής γύρισε την εφαρμογή στα ιταλικά και βρήκε τα υπόλοιπα. Πέντε ήταν κώδικας, ένα
ήταν κείμενο.

**Το SMS.** Το πρότυπο και η κατακλείδα στις Ρυθμίσεις ήταν δεδομένα — τα γράφει ο
γραμματέας — και τα είχα αφήσει ελληνικά επίτηδες. Λάθος: όσο δεν τα έχει πειράξει,
πρέπει να διαβάζονται στη γλώσσα του. Τώρα, αν η τιμή είναι ακόμη η προεπιλογή, ακολουθεί
τη γλώσσα· μόλις γραφτεί κάτι δικό του, μένει όπως γράφτηκε. Μαζί γύρισε και το σώμα του
μηνύματος: «I vostri turni», «ORDINE DELLA GIORNATA», «<-- VOI». Τα πεδία `{ΑΓΩΝΑΣ}`
`{ΗΜΕΡΟΜΗΝΙΑ}` `{ΤΟΠΟΘΕΣΙΑ}` `{ΩΡΑ}` μένουν ελληνικά σε κάθε γλώσσα: είναι κλειδιά
αντικατάστασης, όχι κείμενο.

**Το γλωσσάρι.** Το άρθρο του Κ.Ο.Α.Δ. στην κάρτα κάθε όρου έγραφε «Άρθρο 48» σε κάθε
γλώσσα. Τώρα «Articolo 48». Μαζί γύρισαν οι πέντε επικεφαλίδες ομάδων, οι δύο υποδείξεις
των chip, και **οι είκοσι σημειώσεις** — αυτές που εξηγούν πού διαφωνούν οι κανονισμοί.
Ένας Ιταλός που ανοίγει το γλωσσάρι για να δει γιατί ο οίστρος λείπει από τη στήλη του,
τώρα διαβάζει την απάντηση.

**Το θήραμα** ήταν ελεύθερο κείμενο και έδειχνε «Πέρδικα» σε ιταλική οθόνη. Παραμένει
ελεύθερο — ο γραμματέας γράφει ό,τι πέταξε — αλλά τα πέντε συνηθισμένα διαβάζονται στη
γλώσσα της οθόνης και **ξαναγράφονται ελληνικά στη βάση** μέσω αντίστροφης αναζήτησης σε
όλες τις γλώσσες. Ό,τι δεν αναγνωρίζεται μένει όπως γράφτηκε.

**Η χώρα ακολουθεί τη γλώσσα.** Ένας Ιταλός που καταχωρεί σκύλο έπαιρνε «Κύπρος +357» και
το διόρθωνε κάθε φορά. Τώρα η προεπιλογή είναι Ιταλία +39, Γαλλία +33, Γερμανία +49 — και
η χώρα της γλώσσας στέκει πρώτη στον κατάλογο, με Κύπρο και Ελλάδα αμέσως από κάτω. **Τα
αγγλικά μένουν στην Κύπρο**: είναι η γλώσσα επισκεπτών κάθε εθνικότητας, όχι μιας χώρας,
και η λέσχη είναι κυπριακή. Αλλάζει με ένα πάτημα· ορίζει μόνο την προεπιλογή.

**Τα μηνύματα.** Εβδομήντα επτά `toast`, `alert` και `confirm` — «Αποθηκεύτηκε», «Ο σκύλος
αποκλείστηκε», «Διαγραφή ΟΛΩΝ των δεδομένων;» — δεν περνούσαν ποτέ από το `tx()`. Δεν τα
είχε αναφέρει κανείς, γιατί φαίνονται μια στιγμή και χάνονται· είναι όμως η μόνη απάντηση
που παίρνει ο κριτής όταν πατά κάτι. Τώρα γυρίζουν κι αυτά.

Το λεξικό πήγε από 675 σε **786 κλειδιά**, 3.144 μεταφράσεις. Ο έλεγχος `gaps.js` έμαθε
να ξεσκεπάζει και τα `\n` μέσα στα κλειδιά, που τα έκρυβαν από τη σάρωση.

**Δύο τυπογραφικά που έπιασε μόνο η οθόνη.** Στα γερμανικά τα δύο κουμπιά της μπάρας
έλεγαν και τα δύο «Laden» — τώρα ↓ *Sichern* και ↑ *Laden*. Και το ελληνικό «ζεύγος 1ο»
έχασε το τακτικό του όταν η φράση έγινε μεταφράσιμη· επανήλθε, γιατί το «ο» είναι
ελληνικός τύπος και δεν μεταφέρεται σε άλλη γλώσσα.

**Το σήμα.** Η εφαρμογή δεν έχει δικό της — φοράει το έμβλημα του ομίλου. Το
`LOGO-BRIEF.md` δίνει οκτώ ονόματα από το λεξιλόγιο του κυνηγιού με τα υπέρ και τα κατά
του καθενός, πρόταση τη **ΦΕΡΜΑ**, και έτοιμο κείμενο για το Claude Design — μαζί με τον
όρο ότι το έμβλημα του Κ.Ο.Α.Δ. στέκει πάντα δίπλα, ως ιδιοκτήτης.

**Και μια απόφαση που πήρα λάθος.** Είχα κρατήσει τα πεδία του προτύπου SMS ελληνικά σε
κάθε γλώσσα — `{ΑΓΩΝΑΣ}` `{ΗΜΕΡΟΜΗΝΙΑ}` `{ΤΟΠΟΘΕΣΙΑ}` `{ΩΡΑ}` — με το σκεπτικό ότι είναι
κλειδιά αντικατάστασης, όχι κείμενο. Σωστό για τη μηχανή, άχρηστο για τον άνθρωπο: ο
γραμματέας πρέπει να τα διαβάσει για να τα βάλει στη θέση τους. Τώρα διαβάζονται στη
γλώσσα της οθόνης — `{PROVA}`, `{EPREUVE}`, `{PRUEFUNG}` — και γράφονται πάντα ελληνικά
στη βάση. Η αντικατάσταση δέχεται και τις πέντε μορφές, ώστε πρότυπο γραμμένο στα ιταλικά
να δουλεύει σε ελληνικό κινητό μετά από συγχώνευση. Και ένα «Αποθήκευση» χωρίς αλλαγή δεν
παγώνει το πρότυπο στη γλώσσα που τύχαινε να είναι ανοιχτή: αν το κείμενο είναι ακόμη η
προεπιλογή, αποθηκεύεται η προεπιλογή. `i18n/tools/sms.js` το ελέγχει και στις τέσσερις.
